...
export class AppComponent implements OnInit {
  ...
  getInitalValueMax() {
    return (this.Form.get('range') as FormArray).controls[0].value
      ? (this.Form.get('range') as FormArray).controls[0].get('max').value
      : 'null';
  }
  getInitalValueMin() {
    return (this.Form.get('range') as FormArray).controls[0].value
      ? (this.Form.get('range') as FormArray).controls[0].get('min').value
      : 'null';
  }
  getInitalValueMaxTrue() {
    return (this.Form.get('range') as FormArray).controls[0].value
      ? (this.Form.get('range') as FormArray).controls[0].value
      : 'null';
  }
